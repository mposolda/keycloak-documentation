...
OPTIONAL execution requirement removed::
    ...
    replacing it with CONDITIONAL requirement, which allows more flexibility. The existing OPTIONAL authenticators configured in previous version will be replaced with the CONDITIONAL subflows, where particular subflow will have
    the `Conditional Block - User Configured` condition configured as first execution and the particular authenticator (for example `OTP Form`) as second execution.
    From the user point of view, the behaviour during authentication should be same as in previous version.
...
Changes in the Java SPI::
    ...
    may be affected if your authenticator supported OPTIONAL execution requirement. It is recommended to doublecheck latest authentication
...
Freemarker template changes::
   ...
   templates for login forms or some account forms. Especially the forms related to OTP. It is recommended to doublecheck changes in
