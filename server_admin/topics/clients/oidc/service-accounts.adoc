...
Only the access token is returned by default. No refresh token is returned and also no user session is created
on the {project_name} side upon successful authentication by default. Due the lack of refresh token, there is a need to re-authenticate when the access token expires,
however this does not mean any additional overhead on the {project_name} server side because sessions are not created by default.
